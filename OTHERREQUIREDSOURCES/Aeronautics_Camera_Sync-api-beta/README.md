...
ACS moves the point the player actually looks *from*. Most mods need no changes — any ray that
goes through `level.clip()` is corrected automatically. If yours aims at the wrong place on a
tilting deck, or you need ACS to stand aside, see the **[mod developer guide](docs/API.md)**.

```gradle
repositories { maven { url = "https://api.modrinth.com/maven" } }
dependencies { compileOnly "maven.modrinth:aero_cam_sync:1.3.7" }
```
