...
public class Config {
    ...
    // Третье лицо больше не настройка игрока: им управляет чужой мод через
    // AcsHandle.enableThirdPerson. Ключ allow3rdPerson удалён в схеме 4.
...
//                            o -> o instanceof String);
    ...
    // Временный аварийный откат на время обкатки нового пика. Новый путь задаёт начало луча
    // один раз (PickScope + SableEyeMixin) и ничего не перезаписывает; старый переписывал
    // mc.hitResult целиком и сдвигал ЛЮБОЙ клип рядом с игроком (Issue #30). Убрать вместе
    // со старыми классами, когда новый путь подтверждён живьём в обоих режимах.
    public static final ModConfigSpec.BooleanValue LEGACY_PICK =
            BUILDER.comment(
                    "Fall back to the old pick pipeline (full re-pick + global clip shifting).\n" +
                            "Only for diagnosing regressions: it overwrites other mods' pick results\n" +
                            "and shifts unrelated raycasts near the player.")
                    .define("legacyPick", false);


    // ── Meta ──────────────────────────────────────────────────────────────────
    public static final int CURRENT_CONFIG_SCHEMA = 4; // breaking changes
